...
% Final draft only
\title[]{State of Art on CDMA multiple-access interference mitigation technology} 
...
%% ------------------------------------------------------------
\begin{abstract}
	Code-division multiple access (CDMA) is an algorithm for multiple users to share the same channel simultaneously.
	As such it comes with several drawbacks, mainly multiple-access interferance (MAI). MAI mainly manifests as four
	different problems: The near-far problem, where data from users further from the base station cannot be received
	and decoded correctly. The out-of-phase chips problem, where different data is being sent out of phase, making
	the data impossible to decode. Multipath channel distortion, on which echoes of the signal interfere with the
	channel negatively. And finally the doppler effect, which worsens data retreival by users moving in space.
	The most effective way of mitigating MAI currently, is with Rake CDMA and PN codes, to mitigate out-of-phase chips,
	multipath channel distortion and a bit of the doppler effect. Space-time coded (STC) CDMA systems, in order to mitigate the near-far problem and
	line of sight (LOS) issues. Multiuser detectors (MUDS) for further mitigating the near-far problem as well as some problems
	that come with PN codes. And finally Power Control, in order to completely mitigate the near-far problem, as the
	near-far problem is the most destructive in CDMA.
\end{abstract}
%% ------------------------------------------------------------
...
Furthermore, as it tracks out of phase and lower amplitude replicas, it can also diminish the doppler effect in the received signal.
...
The worst problems in CDMA are the near-far problem and the out-of-phase chips problem.
The out-of-phase chips problem can mostly be mitigated by PN codes, of which their new
quasi-orthogonal problem can also be mitigated by MUDS.
The near-far problem, can be mitigated most reliably by Power Control, but also by MUDs
and a bit by STC CDMA and the Rake system.

Multipath channel distortion, which is a real issue, although a lesser one, can be almost completely mitigated by
the Rake system alone, and turned into further gain, mitigating the near-far problem as stated previously.

For downlink transmission, there is essentially no transmission issues, as all that is required is to send all data
in orthogonal welsh codes through chip vectors. However, for uplink transmission, a mixture of all the systems and
algorithms must be used in order to achieve efficient communication.
